                   ...
                   std::unordered_map<const FaceInfo *, T> * const face_to_value_cache = nullptr)
{
  ...
  try
  {
    ...
    auto action_functor = [&volume_set,
                           ...
                           coord_type,
                           rz_radial_coord,
                           correct_skewness](const Elem & libmesh_dbg_var(functor_elem),
                                             const Elem *,
      ...
      if (functor.isExtrapolatedBoundaryFace(*fi))
      {
        ...
        grad_b += surface_vector * functor(makeCDFace(*fi, correct_skewness));
  ...
  catch (libMesh::LogicError &)
  {
    ...
    mooseAssert(two_term_boundary_expansion,
                ...
                "being used");
    const auto grad =
        greenGaussGradient(elem, functor, false, correct_skewness, mesh, face_to_value_cache);
