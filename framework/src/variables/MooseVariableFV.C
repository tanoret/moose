    ...
    const std::tuple<const FaceInfo *, Moose::FV::LimiterType, bool, bool> & face) const
{
  ...
  mooseAssert(std::get<1>(face) == Moose::FV::LimiterType::CentralDifference,
              "This method currently only supports central differencing.");

  const auto elem_value = getElemValue(&fi->elem());

  return getInternalFaceValue(fi->neighborPtr(), *fi, elem_value, std::get<3>(face));
}
...
MooseVariableFV<OutputType>::adGradSln(const Elem * const elem, const bool correct_skewness) const
{
  ...
  auto grad = Moose::FV::greenGaussGradient(
      elem, *this, _two_term_boundary_expansion, correct_skewness, this->_mesh, &_face_to_value);
...
{
  return evaluateFaceHelper(std::make_tuple(
      std::get<0>(face), std::get<1>(face), std::get<2>(face), false, std::get<3>(face)));
}
