    ...
    _band_frequency_width(getParam<std::vector<Real>>("band_frequency_width"))
{
  ...
  mooseError("Number of variables in the absorptivities vector is not the same than the psi_1 vector!");
...
FVSP3TemperatureSourceSink::computeQpResidual()
{
  ...
  const auto face_arg = Moose::FaceArg({_face_info, LimiterType::CentralDifference, true, false, nullptr, nullptr});

  ADReal thermal_flux(0.0);
  for (std::size_t index = 0; index < _psi1_vec_functors.size(); ++index)
  {
      const auto band_grad_phi_1 = _a1 * _psi1_vec_functors[index]->gradient(face_arg, state) * _face_info->normal();
