...
class TestRC(unittest.TestCase):
    def test(self):
        df1 = run_spatial('1d-rc-continuous.i', 6, y_pp=['L2u', 'L2p'], mpi=8)
...
class TestRC_2D(unittest.TestCase):
    def test(self):
        df1 = run_spatial('2d-rc-continuous.i', 5, y_pp=['L2u', 'L2v', 'L2p'], mpi=16)
...
class TestReconstructed(unittest.TestCase):
    def test(self):
        df1 = run_spatial('reconstructed.i', 7, y_pp=['L2u', 'L2v', 'L2p'], mpi=8)

        fig = mms.ConvergencePlot(xlabel='Element Size ($h$)', ylabel='$L_2$ Error')
        fig.plot(df1, label=['L2u', 'L2v', 'L2p'], marker='o', markersize=8, num_fitted_points=3, slope_precision=1)
        fig.save('reconstructed.png')
        for key,value in fig.label_to_slope.items():
            print("%s, %f" % (key, value))
            self.assertTrue(fuzzyAbsoluteEqual(value, 2., .1))
