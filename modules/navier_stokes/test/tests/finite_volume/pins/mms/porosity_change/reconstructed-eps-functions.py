...
f_u, e_u = mms.evaluate('div(vel*rho*u/porosity) - div(mu*grad(u/porosity)) + porosity*grad(p).dot(e_i)', u, variable='u', vel=vel, p=p, porosity=porosity, scalars=['mu', 'rho'])
f_v, e_v = mms.evaluate('div(vel*rho*v/porosity) - div(mu*grad(v/porosity)) + porosity*grad(p).dot(e_j)', v, variable='v', vel=vel, p=p, porosity=porosity, scalars=['mu', 'rho'])
