...
INSFVRhieChowInterpolator::validParams()
{
  ...
  params.addParam<bool>("correct_volumetric_force", false, "Flag to activate volume corrections.");
                                       ...
                                       const THREAD_ID tid) const
{
  ...
  // Function that allows us to mark the face for which the Rhie-Chow interpolation is inconsistent
  // Normally, we should apply a reconstructed volume correction to the Rhie-Chow coefficients
  // However, since the fluxes at the face are given by the volume force
  // we will simply mark the face add the reverse pressure interpolation for these faces
  // In brief, this function is just marking the faces where the Rhie-Chow interpolation is
  // inconsistent
      ...
      [this, &elem, &neighbor, &time, &fi, &correct_skewness](Point & loc_normal)
  {
    ...
    // it should follow the same process than Green Gauss but for a volume force
    ...
    for (const auto side : make_range(elem->n_sides()))
    {
      ...
      {
        const Elem * const boundary_elem = hasBlocks(elem->subdomain_id()) ? elem : loc_neighbor;
        ...
        for (const auto i : make_range(_volumetric_force.size()))
          elem_value += (*this->_volumetric_force[i])(loc_boundary_face, time) *
                        fi_loc->faceArea() *
    ...
    return std::abs(interp_vf - uncorrected_interp_vf) /
                       std::abs(interp_vf - uncorrected_interp_vf + 1.0) >
                   1e-10
               ? 1.0
               : 0.0;
  };
