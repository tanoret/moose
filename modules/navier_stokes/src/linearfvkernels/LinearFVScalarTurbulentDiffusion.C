...
#include "LinearFVScalarTurbulentDiffusion.h"
#include "Assembly.h"
...
registerMooseObject("MooseApp", LinearFVScalarTurbulentDiffusion);

InputParameters
LinearFVScalarTurbulentDiffusion::validParams()
{
...
LinearFVScalarTurbulentDiffusion::LinearFVScalarTurbulentDiffusion(const InputParameters & params)
...
void
LinearFVScalarTurbulentDiffusion::initialSetup()
{
...
Real
LinearFVScalarTurbulentDiffusion::computeElemMatrixContribution()
{
...
Real
LinearFVScalarTurbulentDiffusion::computeNeighborMatrixContribution()
{
...
Real
LinearFVScalarTurbulentDiffusion::computeElemRightHandSideContribution()
{
  const Elem * elem = _current_face_info->elemPtr();
  const auto bounded_elem = _wall_bounded.find(elem) != _wall_bounded.end();
...
Real
LinearFVScalarTurbulentDiffusion::computeNeighborRightHandSideContribution()
{
  const Elem * neighbor = _current_face_info->neighborPtr();
  const auto bounded_neigh = _wall_bounded.find(neighbor) != _wall_bounded.end();
...
Real
LinearFVScalarTurbulentDiffusion::computeBoundaryMatrixContribution(const LinearFVBoundaryCondition & bc)
{
  const Elem * elem = _current_face_info->elemPtr();
  const auto bounded_elem = _wall_bounded.find(elem) != _wall_bounded.end();
...
Real
LinearFVScalarTurbulentDiffusion::computeBoundaryRHSContribution(const LinearFVBoundaryCondition & bc)
{
  const Elem * elem = _current_face_info->elemPtr();
  const auto bounded_elem = _wall_bounded.find(elem) != _wall_bounded.end();
