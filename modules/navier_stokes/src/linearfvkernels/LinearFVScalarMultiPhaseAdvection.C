...
#include "LinearFVScalarMultiPhaseAdvection.h"
#include "MooseLinearVariableFV.h"
#include "NSFVUtils.h"
#include "NS.h"

registerMooseObject("NavierStokesApp", LinearFVScalarMultiPhaseAdvection);

InputParameters
LinearFVScalarMultiPhaseAdvection::validParams()
{
  ...
  params.addRequiredParam<MooseFunctorName>("alpha", "The Eulerian phase fraction.");
...
LinearFVScalarMultiPhaseAdvection::LinearFVScalarMultiPhaseAdvection(const InputParameters & params)
...
Real
LinearFVScalarMultiPhaseAdvection::computeElemMatrixContribution()
{
  return _advected_interp_coeffs.first * _volumetric_face_flux * _current_face_area;
}

Real
LinearFVScalarMultiPhaseAdvection::computeNeighborMatrixContribution()
{
  return _advected_interp_coeffs.second * _volumetric_face_flux * _current_face_area;
}

Real
LinearFVScalarMultiPhaseAdvection::computeElemRightHandSideContribution()
{
  return 0.0;
}

Real
LinearFVScalarMultiPhaseAdvection::computeNeighborRightHandSideContribution()
{
  return 0.0;
}

Real
LinearFVScalarMultiPhaseAdvection::computeBoundaryMatrixContribution(const LinearFVBoundaryCondition & bc)
{
...
Real
LinearFVScalarMultiPhaseAdvection::computeBoundaryRHSContribution(const LinearFVBoundaryCondition & bc)
{
  const auto * const adv_bc = static_cast<const LinearFVAdvectionDiffusionBC *>(&bc);
  mooseAssert(adv_bc, "This should be a valid BC!");
...
void
LinearFVScalarMultiPhaseAdvection::setupFaceData(const FaceInfo * face_info)
{
  ...
  face_arg = Moose::FaceArg{face_info,
                            ...
                            nullptr};
  _volumetric_face_flux *= _alpha(face_arg, state);
