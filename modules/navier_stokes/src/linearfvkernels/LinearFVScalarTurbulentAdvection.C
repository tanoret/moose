...
#include "LinearFVScalarTurbulentAdvection.h"
#include "MooseLinearVariableFV.h"
#include "NSFVUtils.h"
#include "NavierStokesMethods.h"
#include "NS.h"

registerMooseObject("NavierStokesApp", LinearFVScalarTurbulentAdvection);

InputParameters
LinearFVScalarTurbulentAdvection::validParams()
{
...
LinearFVScalarTurbulentAdvection::LinearFVScalarTurbulentAdvection(const InputParameters & params)
...
void
LinearFVScalarTurbulentAdvection::initialSetup()
{
...
Real
LinearFVScalarTurbulentAdvection::computeElemMatrixContribution()
{
...
Real
LinearFVScalarTurbulentAdvection::computeNeighborMatrixContribution()
{
...
Real
LinearFVScalarTurbulentAdvection::computeBoundaryMatrixContribution(const LinearFVBoundaryCondition & bc)
{
  const Elem * elem = _current_face_info->elemPtr();
  const auto bounded_elem = _wall_bounded.find(elem) != _wall_bounded.end();
...
Real
LinearFVScalarTurbulentAdvection::computeBoundaryRHSContribution(const LinearFVBoundaryCondition & bc)
{
  const Elem * elem = _current_face_info->elemPtr();
  const auto bounded_elem = _wall_bounded.find(elem) != _wall_bounded.end();
