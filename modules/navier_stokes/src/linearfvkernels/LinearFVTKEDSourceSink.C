...
LinearFVTKEDSourceSink::computeMatrixContribution()
{
  ...
  else
  {
    ...
    // Compute destruction
    auto destruction = _C2_eps * rho * epsilon / k;

    if (_relaminarization)
    {
      // Useful definitions
      const auto nu = _mu(elem_arg, state) / rho;

      // Friction Reynolds number
      const auto Re_t = Utility::pow<2>(k) / epsilon / nu;
...
LinearFVTKEDSourceSink::computeRightHandSideContribution()
{
  ...
  else
  {
    ...
    const Real rho = _rho(elem_arg, state);
    const Real k = _k(elem_arg, state);
    const Real epsilon = _var.getElemValue(*_current_elem_info, state);
    ...
    Real production = _mu_t(elem_arg, state) * base_strain;
    ...
    if (_relaminarization)
    {
      ...
      const auto prod_increase = _D * f2
                                * (production + 2*_mu(elem_arg, state)*k/Utility::pow<2>(d))
