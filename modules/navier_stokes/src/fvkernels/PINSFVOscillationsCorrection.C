...
PINSFVOscillationsCorrection::validParams()
{
  ...
  params.addRangeCheckedParam<Real>("consistent_scaling", 1e-2,
...
PINSFVOscillationsCorrection::computeQpResidual()
{
  ...
  const auto neighbor_face = _face_info->isBoundary() ? elemFromFace() : neighborFromFace();
  ...
  if (_use_Darcy_friction_model)
  {
    ...
    friction_term_neighbor += (*_cL)(neighbor_face)(_index)*_rho(neighbor_face) / _eps(neighbor_face);
  }
  if (_use_Forchheimer_friction_model)
  {
    friction_term_elem += (*_cQ)(elem_face)(_index)*_rho(elem_face) / _eps(elem_face);
    friction_term_neighbor += (*_cQ)(neighbor_face)(_index)*_rho(neighbor_face) / _eps(neighbor_face);
  }

  Point _face_centroid = _face_info->faceCentroid();
  Point _elem_centroid = _face_info->elemCentroid();
  Point _neighbor_centroid = _face_info->isBoundary() ? _face_info->elemCentroid() : _face_info->neighborCentroid();

  Real geometric_factor =
            _consistent_scaling *
            1. / (_neighbor_centroid - _face_centroid).norm() *
            1. / (_elem_centroid - _face_centroid).norm();
