...
kOmegaSSTF2BlendingAux::computeValue()
{
  ...
  const Moose::StateArg state = Moose::StateArg(1, Moose::SolutionIterationType::Nonlinear);
  const auto k = _k(elem_arg, state);
  const auto omega = _omega(elem_arg, state);
  const auto omega_capped = std::max(_omega(elem_arg, state).value(), 1e-12);
