...
kEpsilonViscosityAux::validParams()
{
  ...
  MooseEnum wall_treatment("eq_newton eq_incremental eq_linearized neq two_layer", "eq_newton");
  params.addParam<MooseEnum>("wall_treatment",
                             wall_treatment,
                             "The method used for computing the wall functions."
                             "'eq_newton', 'eq_incremental', 'eq_linearized', 'neq', 'two_layer'");
...
{
  if (_wall_treatment == NS::WallTreatmentEnum::TWO_LAYER && !_d)
    paramError("wall_distance", "The wall distance should be provided if a two_layer wall treatment is used.");
}

void
kEpsilonViscosityAux::initialSetup()
{
  if (!_wall_boundary_names.empty() && _wall_treatment != NS::WallTreatmentEnum::TWO_LAYER)
  {
...
kEpsilonViscosityAux::computeValue()
{
  ...
  if (_wall_treatment == NS::WallTreatmentEnum::TWO_LAYER)
  {
    const auto k = _k(elem_arg, state);
    const auto epsilon = _epsilon(elem_arg, state);
    const Real dist = (*_d)(elem_arg, state);

    const auto Re_d = std::sqrt(k) * dist / nu;
    const auto A = _delta_Re_y / std::atanh(0.98);
    const auto lambda_blend = 0.5*(1.0 + std::tanh((Re_d-_Re_y_star)/A));

    const auto Te = k / epsilon;
    const auto T = std::max(Te, std::sqrt(mu/epsilon/rho));
    const auto mu_t_keps = rho * _C_mu * k * T;

    const auto mu_2_layer = mu * 0.42 * Re_d * std::pow(_C_mu, 0.25) * (1. - std::exp(-Re_d/70.0));

    return MetaPhysicL::raw_value(lambda_blend * mu_t_keps + mu_2_layer * (1 - lambda_blend));
  }
  else
  {
    ...
    else
    {
      ...
      if (_newton_solve)
        time_scale = _k(elem_arg, state) /
                    std::max(NS::epsilon_low_limit, _epsilon(elem_arg, state));

      const ADReal mu_t_nl =
          _rho(elem_arg, state) * _C_mu * _k(elem_arg, state) * time_scale;
      mu_t = mu_t_nl.value();
      if (_newton_solve)
        mu_t = std::max(mu_t, NS::mu_t_low_limit);
    }
    // Turbulent viscosity limiter
    return std::min(mu_t, _mu_t_ratio_max * MetaPhysicL::raw_value(mu));
  }
}
