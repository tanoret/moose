...
WCNSFV2PInterphaseDragFunctorMaterial::validParams()
{
  ...
  params.addParam<MooseFunctorName>("u_coupled", "The velocity in the x direction for the coupled phase.");
  params.addParam<MooseFunctorName>("v_coupled", "The velocity in the y direction for the coupled phase.");
  params.addParam<MooseFunctorName>("w_coupled", "The velocity in the z direction for the coupled phase.");
  ...
  MooseEnum drag_model("schiller-naumann morsi-alexander", "schiller-naumann");
  params.addParam<MooseEnum>(
      "drag_model", drag_model, "The drag model used.");

  params.addParam<MooseFunctorName>("interphase_drag_name", "interphase_drag", "The name of the interface drag coefficient.");
...
WCNSFV2PInterphaseDragFunctorMaterial::WCNSFV2PInterphaseDragFunctorMaterial(
  ...
  : FunctorMaterial(params),
    ...
    _u_var_coupled(params.isParamValid("u_coupled")
               ? &getFunctor<Real>("u_coupled")
               : nullptr),
    _v_var_coupled(params.isParamValid("v_coupled")
               ? &getFunctor<Real>("v_coupled")
               : nullptr),
    _w_var_coupled(params.isParamValid("w_coupled")
               ? &getFunctor<Real>("w_coupled")
               : nullptr),
    _alpha_main(params.isParamValid("fd_main")
               ? &getFunctor<Real>("fd_main")
               : nullptr),
    _alpha(getFunctor<Real>("fd")),
    _rho(getFunctor<Real>(NS::density)),
    _rho_coupled(getFunctor<Real>(NS::density + "_coupled")),
    _mu(getFunctor<Real>(NS::mu)),
    _mu_coupled(getFunctor<Real>(NS::mu + "_coupled")),
    _particle_diameter(getFunctor<Real>("particle_diameter")),
    ...
    _drag_model(getParam<MooseEnum>("drag_model"))
{
  ...
  addFunctorProperty<Real>(
    getParam<MooseFunctorName>("interphase_drag_name"),
