...
dam_dims_x_out = ${fparse 2.0*to_m}
dam_dims_y_out = ${fparse 2.0*to_m}
dam_dims_x_in  = ${fparse 0.5*to_m}
dam_dims_y_in  = ${fparse 0.5*to_m}
...
[Problem]
  linear_sys_names = 'u_system_p1 v_system_p1 u_system_p2 v_system_p2 u_system_p3 v_system_p3
                      pressure_system
                      alpha_1_system alpha_2_system alpha_3_system'
...
[UserObjects]
  ...
  [rc_p3]
    ...
    pressure = pressure
    rho = ${rho3}
    p_diffusion_kernel = p_diffusion_p3
    check_executioner = false
    alpha = 'alpha_3'
    property_suffix = 'p3'
  []
...
[Variables]
  ...
  [vel_x_p3]
    type = MooseLinearVariableFVReal
    solver_sys = u_system_p3
    initial_condition = 0.0
  []
  [vel_y_p3]
    type = MooseLinearVariableFVReal
    solver_sys = v_system_p3
    initial_condition = 0.0
  []
  [pressure]
    type = MooseLinearVariableFVReal
...
[LinearFVKernels]
  ...
  [u_advection_stress_p3]
    ...
    v = vel_y_p3
    momentum_component = 'x'
    rhie_chow_user_object = 'rc_p3'
    alpha = 'alpha_3'
    use_nonorthogonal_correction = false
  []
  [v_advection_stress_p3]
    type = LinearWCNSFVMultiPhaseMomentumFlux
    variable = vel_y_p3
    advected_interp_method = ${advected_interp_method}
    mu = ${mu_3}
  ...
  [v_gravity_p3]
    type = LinearFVSource
    variable = vel_y_p3
    source_density = 'alpha_3'
    scaling_factor = ${fparse -rho3 * gravity}
  []
...
[Functions]
  [alpha_1_init]
    type = ParsedFunction
    expression = 'if(((x > ${dam_dims_x_in} | y > ${dam_dims_x_in}) & x < ${dam_dims_x_out} & y < ${dam_dims_x_out}), 1.0, 0.0)'
  []
  [alpha_2_init]
    type = ParsedFunction
    expression = 'if((x > ${dam_dims_x_out} | y > ${dam_dims_y_out}), 1.0, 0.0)'
  []
  [alpha_3_init]
    type = ParsedFunction
    expression = 'if((x < ${dam_dims_x_in} & y < ${dam_dims_y_in}), 1.0, 0.0)'
  []
[]
...
[Executioner]
  ...
  rhie_chow_user_objects = 'rc_p1 rc_p2 rc_p3'
  momentum_systems = 'u_system_p1 v_system_p1;
                      u_system_p2 v_system_p2;
                      u_system_p3 v_system_p3'
  pressure_system = 'pressure_system'
  phase_systems = 'alpha_1_system alpha_2_system alpha_3_system'
